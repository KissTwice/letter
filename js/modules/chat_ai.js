...
async function getAiReply(chatId, chatType, isBackground = false) {
    ...
    try {
        ...
        // ----- 新增：从 db.apiSettings 获取主机、路径、代理 -----
const apiHost = db.apiSettings.apiHost || '';
const apiPath = db.apiSettings.apiPath || '';
const corsProxy = db.apiSettings.corsProxy || '';
let fullUrl = apiHost + apiPath;
if (!fullUrl) fullUrl = db.apiSettings.url; // 兼容旧数据
...
let endpoint;
if (provider === 'gemini') {
    endpoint = `${fullUrl}/v1beta/models/${model}:generateContent?key=${getRandomValue(key)}`;
} else {
    endpoint = `${fullUrl}/v1/chat/completions`;
}
